...
def index():
    ...
    user = servise.get_user(id=user_id)
    print("got user///")
    print(user)
    if request.method == 'POST':
        ...
        datas = servise.get(state=state, user_id=user_id, conditions=conditions)
    else:
        datas = servise.get(user_id=user_id)
...
def add():
    ...
    if request.method == 'POST':
        ...
        keywords = []
    no_problem = servise.add(user_id=user_id, keywords=keywords)
...
def change(id, state):
    ...
    if request.method == 'GET':
        no_problem = servise.changeStatus(state=state, id=id, user_id=user_id)
    return redirect(url_for('index', user_id=user_id))
...
def login():
    ...
    user  =servise.get_user(name=name)
    if user:
        user_id=user.id
    else:
        user_id = -1
...
def user():
    ...
    if request.method == 'POST':
        ...
        user_id = servise.add_user(name, userid, passwd)
        return redirect(url_for('index', user_id=user_id))
    else:
        user = servise.get_user(id=user_id)
